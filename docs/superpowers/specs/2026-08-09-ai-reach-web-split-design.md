...
- 当前用户信息与权限加载
- 登录时选择租户；登录后不提供动态租户切换
- 用户菜单与退出登录
- 通用通知中心、未读数量和消息已读操作
- AI Call 固定菜单及权限过滤
- 403、404 和运行时错误页面

通知中心继续使用 RuoYi 消息接口和全局 SSE，但不承载 AI Call 坐席事件。新项目不迁移 Recov 的工作区菜单、流程浮层和其他非 AI Call 全局副作用。AI Call 菜单由新项目固定维护，通过当前用户的权限字符串决定可见性和路由访问。
...
登录页继续通过现有租户列表选择目标租户，登录成功后以 Token 中的 `tenantId` 作为 RuoYi 与 AI Call 的共同租户上下文。登录后不提供动态租户切换；需要更换租户时退出登录，并在登录页重新选择租户获取新的 Token。

现有 RuoYi 动态租户接口只改变 Java 后端上下文，不会更新 AI Call 后端校验的 JWT `tenantId`。在后端提供经过服务端授权的租户切换令牌前，不得通过浏览器自定义 Header 覆盖 AI Call 租户。
...
现状中音色接口已明确校验 `ai_call:voice:manage`，多数其他 AI Call 接口主要校验登录和租户。首版拆分和发布继续使用现有登录校验与租户隔离，不要求本次同步补齐细粒度服务端权限。以下权限映射作为 `ed81/ai-call` 的后续安全加固任务，不阻塞 `ai-reach-web` 首版发布：
...
所有业务接口仍须执行登录校验和租户隔离。前端权限控制不能替代服务端校验；在后续加固完成前，细粒度权限只限制前端菜单、路由和操作入口。
...
| 前缀 | 用途 | 上游 |
| --- | --- | --- |
| `/dev-api` | 登录、用户、租户、权限和通用通知 | 现有 RuoYi 后端 |
...
- RuoYi 全局 SSE：`/dev-api/resource/sse`，用于通知中心的系统消息和业务通知刷新
- AI Call 坐席 SSE：`/ai-call-agent-api/ai-call/agent-console/events`，用于转人工、坐席状态和实时通话工作事件

通知中心迁移现有消息列表、未读数量、单条已读和全部已读接口。两条 SSE 都使用当前登录 Token，Nginx 分别关闭响应缓冲并设置长连接超时，但前端不混用事件类型和状态。
...
2. 迁入登录、请求、用户、登录时租户选择和权限基础能力。
3. 迁入通知中心、RuoYi 消息 service 和全局 SSE 适配层。
4. 建立固定 AI Call 菜单和路由。
5. 迁入 13 个入口及全部子页面、service、样式和测试。
6. 修正 Recov 专用导入，使新项目内部依赖闭合。
...
4. 分别选择两个租户重新登录，验证列表与租户隔离。
5. 验证通知中心、两条 SSE、录音、对话、质检、跟进和浏览器媒体能力。
...
- 普通租户不能读取其他租户数据
- 超级管理员在登录页选择不同租户重新登录后，只能读取新 Token 对应租户的数据
- 任务、记录、音色、线路、规则、统计、坐席、跟进和 Lab 页面通过功能验收
- SSE 正常连接，异常时能降级且不会无限重连
- 通知中心能够加载消息、显示未读数量并完成单条及全部已读
- 录音、对话、语义分析和质检链路可用
...
- 登录校验、租户隔离及前端菜单和路由权限正常；后端细粒度权限按第 5.3 节后续加固
